...
Module.register("MMM-envcan",{
	...
	defaults: {
		feeds: [
			{
				title: "Winnipeg",
				url: "https://weather.gc.ca/rss/city/mb-38_e.html",
	...
	extractBoldParam: function(str,token) {
		var pos = str.search("<b>"+token+"</b>");
		if(pos==-1) return "";

		str = str.slice(pos);
		var value = str.slice(str.search("</b>")+4,str.search("<br/>"));
		return value.trim();
	},
	...
	getDom: function() {
		...
		if (this.newsItems.length > 0) {
			...
			if(this.newsItems[this.activeItem].title.search("Current Conditions")!=-1) {
				// store parsed current conditions
				this.cachedcurrentconditions = this.newsItems[this.activeItem].description;
				
				// todo, parse through the cdata:
				/*  <summary type="html"><![CDATA[<b>Observed at:</b> Winnipeg Richardson Int'l Airport 9:00 PM CST Monday 04 February 2019 <br/>
					<b>Condition:</b> Drifting Snow <br/>
					<b>Temperature:</b> -24.7&deg;C <br/>
					<b>Pressure / Tendency:</b> 103.0 kPa rising<br/>
					<b>Visibility:</b> 24.1 km<br/>
					<b>Humidity:</b> 65 %<br/>
					<b>Wind Chill:</b> -38 <br/>
					<b>Dewpoint:</b> -29.5&deg;C <br/>
					<b>Wind:</b> N 25 km/h<br/>
					<b>Air Quality Health Index:</b> N/A <br/>
					]]></summary>
				*/
				var condition = this.extractBoldParam( this.cachedcurrentconditions, "Temperature:" );
				console.log("Temperature="+condition);
				// todo, display graphic for condition
				var condition = this.extractBoldParam( this.cachedcurrentconditions, "Condition:" );
				console.log("Condition="+condition);

			}

			if(this.config.stickyCurrentConditions && this.newsItems[this.activeItem].title.search("Current Conditions")==-1) {
				...
				currentconditions.className = "xsmall light";
				currentconditions.innerHTML = this.cachedcurrentconditions+"<br>";
		...
		for (var feed in feeds) {
			...
			if (this.subscribedToFeed(feed)) {
				for (var i in feedItems) {
					...
					console.log("item.title.search:"+item.title+' '+item.title.search("Current Conditions"));
					if(item.title.search("Current Conditions")!=-1) {
						console.log("Current conditions found a "+i);
						//currentConditionsIndex=i;
					}
